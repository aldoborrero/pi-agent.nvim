-- Tests for file refresh functionality in Pi Agent
...
describe('file refresh', function()
  ...
  describe('setup', function()
    it('should create an augroup for file refresh', function()
      file_refresh.setup(pi_agent, config)

      assert.is_not_nil(registered_augroups['PiAgentFileRefresh'], 'File refresh augroup should be created')
      assert.is_true(registered_augroups['PiAgentFileRefresh'].clear, 'Augroup should be cleared on creation')
    end)
    ...
    it('should save the current updatetime', function()
      ...
      assert.are.equal(4000, pi_agent.pi_agent.saved_updatetime, 'Should save the current updatetime')
    end)
    ...
    it('should not setup refresh when disabled in config', function()
      ...
      assert.is_nil(registered_augroups['PiAgentFileRefresh'], 'Augroup should not be created when refresh is disabled')
    end)
  end)
