---@mod pi-agent.terminal Terminal management for pi-agent.nvim
---@brief [[
--- This module provides terminal buffer management for pi-agent.nvim.
...
-- @table PiAgentTerminal
-- @field instances table Key-value store of git root to buffer number
-- @field saved_updatetime number|nil Original updatetime before Pi Agent was opened
...
--- Create a floating window for Pi Agent
...
--- @return string|nil cwd Working directory path or nil to use the default
--- @private
local function get_terminal_cwd(config, git)
...
local function generate_buffer_name(instance_id, config)
  if config.git.multi_instance then
    return 'pi-agent-' .. instance_id:gsub('[^%w%-_]', '-')
  else
    return 'pi-agent'
  end
end
...
--- Set up function to force insert mode when entering the Pi Agent window
--- @param pi_agent table The main plugin module
--- @param config table The plugin configuration
function M.force_insert_mode(pi_agent, config)
  local current_bufnr = vim.fn.bufnr('%')

  -- Check if current buffer is any of our Pi Agent instances
  local is_pi_agent_instance = false
  for _, bufnr in pairs(pi_agent.pi_agent.instances) do
    if bufnr and bufnr == current_bufnr and vim.api.nvim_buf_is_valid(bufnr) then
      is_pi_agent_instance = true
      break
    end
  end

  if is_pi_agent_instance then
...
local function is_valid_terminal_buffer(bufnr)
  ...
  return buftype == 'terminal' and terminal_job_id and vim.fn.jobwait({ terminal_job_id }, 0)[1] == -1
end
...
local function handle_existing_instance(bufnr, config)
  ...
  if #win_ids > 0 then
    -- Pi Agent is visible, close the window
    for _, win_id in ipairs(win_ids) do
      vim.api.nvim_win_close(win_id, true)
    end
  else
    -- Pi Agent buffer exists but is not visible, open it in a split or float
...
--- Create new Pi Agent instance
--- @param pi_agent table The main plugin module
--- @param config table Plugin configuration
--- @param git table Git module
--- @param instance_id string Instance identifier
--- @param command_override string|nil Optional command to use instead of config.command
--- @private
local function create_new_instance(pi_agent, config, git, instance_id, command_override)
  local cmd = command_override or config.command
  local cwd = get_terminal_cwd(config, git)
  local term_opts = {}
  if cwd then
    term_opts.cwd = cwd
  end
  ...
  if config.window.position == 'float' then
    ...
    pi_agent.pi_agent.instances[instance_id] = new_bufnr
  else
    -- Regular split window
    create_split(config.window.position, config)

    -- Create buffer and open terminal with termopen
    ...
    -- Store buffer number for this instance
    pi_agent.pi_agent.instances[instance_id] = bufnr
  end
...
--- @param git table The git module
--- @param command_override string|nil Optional command to use instead of config.command
function M.toggle(pi_agent, config, git, command_override)
  ...
  -- Check if this Pi Agent instance is already running
  local bufnr = pi_agent.pi_agent.instances[instance_id]

  -- Validate existing buffer
  if bufnr and not is_valid_terminal_buffer(bufnr) then
    -- Buffer is no longer a valid terminal, reset
    pi_agent.pi_agent.instances[instance_id] = nil
    bufnr = nil
  end
  ...
  if bufnr and vim.api.nvim_buf_is_valid(bufnr) then
    ...
    -- Create new instance
    create_new_instance(pi_agent, config, git, instance_id, command_override)
  end
end
